...
import { HsnTableRow, HsnTableToolbar } from 'sections/@dashboard/hsn';
...
const TABLE_HEAD = [
    ...
    { id: 'hsnCode', label: 'HSN CODE', align: 'left' },
    { id: '' },
];
...
export default function HsnListPage() {
    ...
    const { data, isLoading: gstIsLoading, isError: gstIsError } = useGetAllHsn();
    ...
    const handleEditRow = (id) => {
        navigate(PATH_DASHBOARD.hsn.edit(id));
    };
            ...
            <Container maxWidth={themeStretch ? false : 'lg'}>
                <CustomBreadcrumbs
                    ...
                    links={[
                        ...
                        { name: 'HSN', href: PATH_DASHBOARD.hsn.root },
                        { name: 'HSN List' },
                    ]}
                    action={
                        <Button
                            component={RouterLink}
                            to={PATH_DASHBOARD.hsn.new}
                            variant="contained"
...
function applyFilter({ inputData, comparator, filterName }) {
    ...
    if (filterName) {
        inputData = inputData?.filter(
            (item) => item.hsnCode?.toLowerCase().indexOf(filterName?.toLowerCase()) !== -1
        );
    }
