...
import { useCreateNotification, useUpdateNotificationById } from 'services/notificationServices';
...
NotificationAddForm.propTypes = {
  isEdit: PropTypes.bool,
  notificationData: PropTypes.func,
};

export default function NotificationAddForm({ isEdit = false, notificationData }) {
  ...
  const { createNotification, isLoading: notiIsLoading } = useCreateNotification();
  const { updateNotification, isLoading: updatenotiIsLoading } = useUpdateNotificationById();
  ...
  const defaultValues = useMemo(
    () => ({
      ...
      bannerType: notificationData?.bannerType ? notificationData?.bannerType : '',
      attachFile: notificationData?.attachFile ? notificationData?.attachFile : '',
      youtubeLink: notificationData?.youtubeLink ? notificationData?.youtubeLink : '',
      seoTitle: notificationData?.seoTitle || '',
      seoMetaDescription: notificationData?.seoMetaDescription || '',
      categoryName: notificationData?.categoryName || '',
      seoTags: notificationData?.seoTags || [process.env.REACT_APP_COMPANY_NAME],
      seoSlug: notificationData?.seoSlug || '',
      title: notificationData?.title ? notificationData?.title : '',
      shortDescription: notificationData?.shortDescription ? notificationData?.shortDescription : '',
      redirection_noti: notificationData?.redirection_noti || true,
      redirection_link: notificationData?.redirection_link ? notificationData?.redirection_link : '',
      descriptions: notificationData?.descriptions ? notificationData?.descriptions : [],
      postedBy: notificationData?.postedBy ? notificationData?.postedBy : '',
      userType: notificationData?.userType ? notificationData?.userType : '',
    }),
    [notificationData]
  );
  ...
  useEffect(() => {
    if (isEdit && notificationData) {
      reset(defaultValues);
      setBlogFor(notificationData?.bannerType);
      setValue('attachFile', notificationData?.attachFile);
    }
  }, [isEdit, notificationData, defaultValues]);
  ...
  const onSubmit = async (data) => {
    ...
    let result = arr1.filter(
      (itemX) =>
        ...
        (itemX?.notificationDescriptions ||
          itemX?.images ||
  ...
  const closeIt = () => {
    reset();
    navigate(PATH_DASHBOARD.notification.list);
  };
  ...
  // useEffect(() => {
  //   notificationData?.descriptions?.map((item, index) => {
  //     setIndexes((prevIndexes) => [...prevIndexes, index]);
  //   });
  // }, [notificationData?.descriptions]);
                  ...
                  <fieldset name={fieldName} key={fieldName} style={{ border: 'none' }}>
                    <div>
                      <MoreData
                        index={index}
                        fieldName={fieldName}
                        descriptions={notificationData?.descriptions}
                        setValue={setValue}
                        values={values}
                        ref={register}
                      />
        ...
        <Grid item xs={12} md={4}>
          ...
          <Card sx={{ mb: 3, mt: 3, p: 2 }}>
            ...
            <Stack spacing={3}>
              ...
              <RHFTextField disabled={true} name="seoSlug" label="Slug (Type in english)" />
              <RHFTextField
