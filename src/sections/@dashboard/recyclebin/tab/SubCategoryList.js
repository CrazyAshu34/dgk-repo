...
import {
  useEmptyAllDataSubCategories,
  useGetAllDeleteSubCategoriesRecyclebin,
  usePermanentlyDeleteSubCategoriesById,
  useReStoreAllDataSubCategories,
  useReStoreDataSubCategoriesById,
} from '../../../../services/subCategoryServices';
import SubCategoryTableRow from './SubCategoryTableRow';
import SubCategoryTableToolbar from './SubCategoryTableToolbar';
...
export default function SubCategoryListPage() {
  ...
  const { ReStoreData } = useReStoreDataSubCategoriesById();
  const { ReStoreAllData } = useReStoreAllDataSubCategories();
  const { PermanentlyDeleteSubCategories } = usePermanentlyDeleteSubCategoriesById();
  const { EmptyAllData } = useEmptyAllDataSubCategories();
  ...
  if (subCategoryIsLoading) return <LoadingScreen />;
  if (subCategoryIsError) return <BlankPage />;
  ...
  const handleDeletePermanentlyRow = (id) => {
    PermanentlyDeleteSubCategories(id);
    const filterData = tableData.filter((item) => item._id !== id);
    setTableData(filterData);
  };
