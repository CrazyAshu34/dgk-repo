...
ShiprocketOrderTableRow.propTypes = {
...
export default function ShiprocketOrderTableRow({ row, index, onDetails, handleOpen, handleOpenCancelModal, handleCodPaymentModal, downloadInvoice, filterStatus, handleExchangeOrderModal, handleReturnOrderModal }) {
    const {
        _id,
        orderId,
        customerDetails,
    ...
    const isOrderDelivered = orderStatusTimeline.some(item => item.status === "DELIVERED");
    ...
    const attemptedStatuses = ["PLACED", "PROCESSING", "PACKED", "SHIPPED", "DELIVERED"];
    const attemptedExchangeStatus = ["EXCHANGE_INITIATED", "EXCHANGE_ACCEPTED", "PRODUCT_PICKED_UP", "PROCESSING", "PACKED", "SHIPPED", "EXCHANGED"];
    const attemptedReturnedStatus = ["RETURN_INITIATED", "RETURN_ACCEPTED", "PRODUCT_PICKED_UP", "PROCESSING", "RETURNED"];
    ...
    const handleChange = (event, updateStatus, orderMode) => {
        ...
        } else if (event.target.value === "PACKED" && updateStatus === "EXCHANGE_INITIATED") {
            handleExchangeOrderModal(orderId, "PACKED", row);
