...
export default function Upload({
  ...
  const hasFiles = files && multiple && !imgArr && files.length > 0;

  const hasImgFiles = files && multiple && imgArr && files.length > 0;
              ...
              item?.values.map((value, i) => (
                ...
                >
                  <Box
                    ...
                    alt="The house from the offer."
                    src={getFormattedImageURL(value?.url)}
                  />
                  <IconButton
                    size="small"
                    onClick={() => onRemove(value)}
...
function Placeholder({ sx, pimg, placeholder, ...other }) {
  return (
    <Stack
      spacing={1}
      alignItems="center"
